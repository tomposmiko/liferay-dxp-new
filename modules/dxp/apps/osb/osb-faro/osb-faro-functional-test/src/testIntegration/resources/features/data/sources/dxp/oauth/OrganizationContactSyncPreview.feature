...
Feature: Preview the number of Organization contacts to sync
	As an Business User, I should be able to see the preview number of Organization contacts to sync
	...
	Background: [Setup] Navigate to the Data Source Page and Click to Add a DXP Data Source
		...
		* I type "OrganizationContactSyncPreview - ${Random.1}" into the "Name" input field
		...
		* I should see that "OrganizationContactSyncPreview - ${Random.1}" was successfully authenticated
		
	Scenario: Preview the number of Organization contacts to sync
		Given I click the "Configure Data Source" tab
		And I click the button for Contacts Configuration
		When I sync the Data Source Contacts by the following Organizations:
			| Gold Team |
		Then I should see a preview that "3" users will be synced